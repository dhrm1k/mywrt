...
VERSION_NUMBER:=$(if $(VERSION_NUMBER),$(VERSION_NUMBER),0.1.0-dev)

VERSION_CODE:=$(call qstrip,$(CONFIG_VERSION_CODE))
MYWRT_VERSION_CODE:=$(shell $(TOPDIR)/scripts/mywrt-version.sh 2>/dev/null)
VERSION_CODE:=$(if $(VERSION_CODE),$(VERSION_CODE),$(if $(MYWRT_VERSION_CODE),$(MYWRT_VERSION_CODE),$(REVISION)))

VERSION_REPO:=$(call qstrip,$(CONFIG_VERSION_REPO))
VERSION_REPO:=$(if $(VERSION_REPO),$(VERSION_REPO),https://downloads.openwrt.org/releases/25.12.5)

VERSION_DIST:=$(call qstrip,$(CONFIG_VERSION_DIST))
VERSION_DIST:=$(if $(VERSION_DIST),$(VERSION_DIST),MyWrt)
VERSION_DIST_SANITIZED:=$(call sanitize,$(VERSION_DIST))

VERSION_MANUFACTURER:=$(call qstrip,$(CONFIG_VERSION_MANUFACTURER))
VERSION_MANUFACTURER:=$(if $(VERSION_MANUFACTURER),$(VERSION_MANUFACTURER),MyWrt)
...
VERSION_PRODUCT:=$(if $(VERSION_PRODUCT),$(VERSION_PRODUCT),MyWrt Router)
